...
public class DragImageView extends ImageView {
	...
	public void setImageBitmap(Bitmap bm) {
		...
		MIN_W = bitmap_W / 2;
		MIN_H = bitmap_H / 2;
		Log.d("tag","MyAsyncTask-new--setimg>"+bitmap_H+"<>"+bitmap_W);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_POINTER_UP:
			mode = MODE.NONE;
			Log.d("tag","drag--pointer-up"+isScaleAnim);
			/** 执行缩放还原 **/
			if (isScaleAnim) {
				doScaleAnim();
			}
	...
	void setScale(float scale) {
		...
		Log.d("tag","scale--left:"+getLeft()+"right:"+this.getRight());
		// 放大
	...
	class MyAsyncTask extends AsyncTask<Void, Integer, Void> {
		...
		public MyAsyncTask(int screen_W, int current_Width, int current_Height) {
			...
			Log.d("tag","MyAsyncTask-new-->"+current_Height+"<>"+current_Width);
		}
...
//			Log.d("tag","doINBack-->"+start_Top+"start>>"+start_Bottom);
			while (current_Width <= screen_W) {
				...
				Log.d("tag","top:"+top+"start-top>"+start_Top);
				left = Math.max(left, start_Left);
				top = Math.max(top, start_Top);//top = Math.max(top, start_Top);
				right = Math.min(right, start_Right);
				bottom = Math.min(bottom, start_Bottom);//bottom = Math.min(bottom, start_Bottom);
			...
			Log.d("tag","doINBack--end>"+DragImageView.this.getHeight()+"screenW>>"+DragImageView.this.getWidth());
			return null;
		}
