...
public class CompanyDetailServlet extends HttpServlet {
	...
	private List<JobBean> query(HttpServletRequest request) throws SQLException{
		...
		String sql = "select job.id,job.name, avg(job.salary),count(job.id)" +
				" from job where companyId = '"+key+"' group by job.name  order by avg(job.salary)";
		...
		while(rs.next()){
			...
			job.setSalary(rs.getInt(1));
			job.setCompanyCount(rs.getInt(2));
			list.add(job);
		}
		return list;
	}
