...
public class CompanySalaryRankServlet extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public CompanySalaryRankServlet() {
		super();
	}
	...
	private List<CompanyBean> query(HttpServletRequest request) throws SQLException{
//		String key = request.getParameter("key");
		String sql = "select companytable.id,companytable.name,avg(job.salary) from companytable " +
				"left join job on companytable.id=job.companyId order by avg(job.salary)";
		...
		while(rs.next()){
			...
			company.setAvgSalary(rs.getInt(1));
			list.add(company);
		}
		return list;
	}
	...
	private JSONObject Json(List<CompanyBean> list) throws JSONException{
		...
		for(int i = 0;i<list.size();i++){
			...
			item.put("salary",list.get(i).getAvgSalary());
			array.put(item);
		}
